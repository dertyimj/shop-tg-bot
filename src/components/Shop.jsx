...
export default function Shop({ user }) {
  ...
  useEffect(() => {
    // Welcome только 1 раз на запуск
    setTimeout(() => setShowWelcome(false), 3500); // fallback на случай if user skip
  }, []);
  ...
  function handlePay() {
    ...
    // Сохраняем заказ (как раньше) в localStorage
    const order = {
      id: Date.now(),
      items: cart,
      createdAt: Date.now(),
    ...
    setShowPaid(true); setTimeout(() => setShowPaid(false), 2000);
  }
  ...
  return (
    <div className="relative min-h-[92vh]">
      ...
      {/* Аватар пользователя */}
      <motion.div
        className="absolute left-4 top-2 z-10 flex items-center"
      ...
      {/* Сетка товаров */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-7 pt-16 px-3 pb-5">
        {items.map((item) => (
        ...
        {selected && (
          ...
          >
            ...
            >
              ...
              <button className="absolute top-2 right-2 text-accent2 hover:text-accent text-2xl active:scale-90" onClick={closeDetails}>&times;</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
